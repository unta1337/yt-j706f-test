...
static int temp_debug = 1000;
static struct class *adc_cali_class;
static int adc_cali_major;
static dev_t adc_cali_devno;
static struct cdev *adc_cali_cdev;
static struct power_supply *bq27541_psy;
static bool g_charger_enabled = true;
#ifndef CONFIG_CHARGE_MACRO_P530
bool g_charging_enabled = true;
static bool g_input_suspend = false;
#endif
extern int batt_maintenance_mode;
...
int get_prop_from_bq27541(enum power_supply_property psp,
	union power_supply_propval *val)
{
	int rc;

	if (!bq27541_psy){
		pr_err("%s bq27541_psy is null!\n",__func__);
		return -EINVAL;
	}
	rc = power_supply_get_property(bq27541_psy, psp, val);
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		g_charger_enabled = val->intval;
		mtk_chaging_enable_write(val->intval);
		break;
#ifndef CONFIG_CHARGE_MACRO_P530
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		lc_chaging_enable_write(val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		g_input_suspend = val->intval;
		lc_input_suspend_write(g_input_suspend);
		break;
#endif
	case POWER_SUPPLY_PROP_TEMP_DEBUG:
		temp_debug = val->intval/10;
		break;
	default:
		ret = -EINVAL;
		break;
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		...
		ret = get_prop_from_bq27541(POWER_SUPPLY_PROP_CYCLE_COUNT, val);
		if (ret < 0) {
			val->intval = gm.bat_cycle;
			ret = 0;
			pr_err("mtk fg_get_bat_cycle:%d\n", val->intval);
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		 val->intval = g_charger_enabled;
		break;
#ifndef CONFIG_CHARGE_MACRO_P530
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		 val->intval = g_charging_enabled;
		break;
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		 val->intval = g_input_suspend;
		break;
#endif
	case POWER_SUPPLY_PROP_BATT_MAINTENANCE:
		if (batt_maintenance_mode == 3) {
			val->intval = 3;
		} else if (batt_maintenance_mode == 2) {
			val->intval = 2;
		} else  if (batt_maintenance_mode == 1)  {
			val->intval = 1;
		} else {
			val->intval = 0;
		}
...
int force_get_tbat_internal(bool update)
{
	...
	static int pre_bat_temperature_val = -1,temp_i = 0;
	int fg_r_value = 0;
	int fg_meter_res_value = 0;
	...
	if (temp_debug < 1000)
		bat_temperature_val=temp_debug;

      if (bat_temperature_val <= -400) {
		  pr_err("Shut down:The battery doesn't exist bat_temperature_val=%d\n",bat_temperature_val);
		  kernel_power_off();
	} else {
		/*ret = get_prop_from_bq27541(POWER_SUPPLY_PROP_TEMP, &val);
		if (ret >= 0) {
			bat_temperature_val = val.intval;
		}*/
		if ((bat_temperature_val < 10) && (bat_temperature_val >= -10)){
			if (temp_i < 30)
				temp_i ++;
			else
...
static int __init battery_probe(struct platform_device *dev)
{
	...
	bq27541_psy = power_supply_get_by_name("bq27541");
