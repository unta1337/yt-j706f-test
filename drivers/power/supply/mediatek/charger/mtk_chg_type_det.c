...
#include "../../mediatek/typec/tcpc/inc/tcpci_typec.h"
...
extern bool charging_pe;
extern u32 mt6360_aicr, mt6360_mivr;
extern struct charger_device *quick_chg_dev;
extern int quick_mt6360_get_aicr(struct charger_device *chg_dev, u32 *uA);
extern int quick_mt6360_get_mivr(struct charger_device *chg_dev, u32 *uV);
...
extern bool is_recheck_charge_type_status(void);
extern bool mtk_is_dual_charge(void);
#endif
	...
	if (!mtk_chg->chg_online) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||
			...
			pr_notice("%s: system_state=%d\n", __func__,
				system_state);
#ifndef CONFIG_CHARGE_MACRO_P530
			if(!is_recheck_charge_type_status())
			{
#endif
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_PE:
		val->intval = charging_pe;
		break;
	default:
		return -EINVAL;
	}
...
extern void batt_protect_charging_enable(bool en);
static int mt_ac_get_property(struct power_supply *psy,
	enum power_supply_property psp, union power_supply_propval *val)
{
	int ret = 0;
	u32 aicr = 0, mivr = 0;
	static bool g_batt_protect_mode_old = false;
	struct mt_charger *mtk_chg = power_supply_get_drvdata(psy);
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if((g_batt_protect_mode != g_batt_protect_mode_old)&&(mtk_chg->chg_type != CHARGER_UNKNOWN)) {
			g_batt_protect_mode_old =g_batt_protect_mode;
			batt_protect_charging_enable(g_batt_protect_mode_old);
		}

		if(quick_chg_dev == NULL){
			pr_info("quick_chg_dev is NULL,mt6360_aicr =%d\n",mt6360_aicr);	
			val->intval = mt6360_aicr;
		} else {
			ret = quick_mt6360_get_aicr(quick_chg_dev, &aicr);
			if (ret < 0) {
				pr_info("quick_chg_dev aicr get fail,aicr =%d,mt6360_aicr=%d\n",aicr,mt6360_aicr);
				val->intval = mt6360_aicr;
			} else {
#ifndef CONFIG_CHARGE_MACRO_P530
				if(mtk_is_dual_charge())
					val->intval = aicr * 2;
				else
#endif
					val->intval = aicr;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if(quick_chg_dev == NULL){
			pr_info("quick_chg_dev is NULL,mt6360_mivr =%d\n",mt6360_mivr);	
			val->intval = mt6360_mivr;
		} else {
			ret = quick_mt6360_get_mivr(quick_chg_dev, &mivr);
			if (ret < 0) {
				pr_info("quick_chg_dev mivr get fail,mivr =%d,mt6360_mivr=%d\n",mivr,mt6360_mivr);
				val->intval = mt6360_mivr;
			} else {
				val->intval = mivr;
			}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION:
		val->intval = power_supply_get_cc();
		break;
	case POWER_SUPPLY_PROP_BATTERY_PROTECT_MODE:
...
enum charger_type mt_get_charger_type(void)
{
	...
	mtk_chg = power_supply_get_drvdata(psy);
#ifdef CONFIG_POGO_SUPPORT
	if((mtk_chg->chg_type == CHARGER_UNKNOWN) && !!is_pogo_connect())
	{
		mtk_chg->chg_type = POGO_CHARGER;
		power_supply_changed(mtk_chg->chg_psy);
		power_supply_changed(mtk_chg->ac_psy);
		power_supply_changed(mtk_chg->usb_psy);
	}
	else if((mtk_chg->chg_type == POGO_CHARGER) && !is_pogo_charging())
	{
		mtk_chg->chg_type = CHARGER_UNKNOWN;
		power_supply_changed(mtk_chg->chg_psy);
	}
#endif
	pr_info("charging_pe =%d,chg_type=%d\n",charging_pe,mtk_chg->chg_type);
