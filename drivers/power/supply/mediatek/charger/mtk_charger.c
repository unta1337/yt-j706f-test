...
extern bool batt_present;
static bool is_batt_protect_below_40 = false;
static bool is_batt_protect_40_to_60 = false;
static bool is_batt_protect_above_60 = false;
bool charging_pe = false;
#ifndef CONFIG_CHARGE_MACRO_P530
bool batt_protect_powerpath = true;
#endif
...
#ifndef CONFIG_CHARGE_MACRO_P530
void lc_charger_manager_enable_power_path(bool en)
{
	int ret = 0;
	bool is_en = true;
	...
	if (pinfo == NULL)
	{
		...
		ret = charger_dev_is_powerpath_enabled(pinfo->chg1_dev,&is_en);
		if (ret < 0) {
			chr_err("%s: get is power path enabled failed\n", __func__);
		} else if (is_en == en) {
			chr_err("%s: power path is already en = %d\n", __func__, is_en);
		} else {
			batt_protect_powerpath = en;
			pr_info("%s: enable power path = %d\n", __func__, en);
...
#ifdef CONFIG_POGO_SUPPORT
int is_pogo_charging(void)
{
	int usb_path_state = 0,pogo_path_state =0;
	if (pinfo == NULL)
		return 0;
	if(pinfo->keyboard_status != 0)
		return 0;

	if(gpio_is_valid(pinfo->lc_usb_int_gpio))
		usb_path_state = gpio_get_value(pinfo->lc_usb_int_gpio) ? 0:1;
	if(gpio_is_valid(pinfo->lc_pogo_int_gpio))
		pogo_path_state = gpio_get_value(pinfo->lc_pogo_int_gpio) ? 0:1;

	if(usb_path_state == 1)
		gpio_direction_output(pinfo->lc_otg_en1_gpio, 1);

	if(get_boot_mode() != KERNEL_POWER_OFF_CHARGING_BOOT){
		pr_info("%s: pinfo->usb_in %d, pinfo->pogo_in %d,usb_int %d, pogo_int %d\n", __func__,
			pinfo->usb_in, pinfo->pogo_in,usb_path_state, pogo_path_state);
		if (((pinfo->otg_in== 1) && (pinfo->pogo_in == 1))
			|| ((pinfo->usb_in== 0) && (pinfo->pogo_in == 1)))
			return 1;
	}else {
		//int usb_path_state = 0,pogo_path_state =0;
		//if(gpio_is_valid(pinfo->lc_usb_int_gpio))
		...
		//if(gpio_is_valid(pinfo->lc_pogo_int_gpio))
		//	pogo_path_state = gpio_get_value(pinfo->lc_pogo_int_gpio) ? 0:1;
		pr_info("%s: usb_int %d, pogo_int %d\n", __func__, usb_path_state, pogo_path_state);
		if (((pinfo->otg_in == 1) && (pogo_path_state == 1))
			|| ((usb_path_state== 0) && (pogo_path_state == 1)))
			return 1;
	}
	return 0;
}
int is_pogo_connect(void)
{
	int usb_path_state = 0,pogo_path_state =0;
	if (pinfo == NULL)
		return 0;
	...
	if(get_boot_mode() != KERNEL_POWER_OFF_CHARGING_BOOT){
		...
		//	pogo_path_state = gpio_get_value(pinfo->lc_pogo_int_gpio) ? 0:1;
		pr_info("%s: usb_int %d, pogo_int %d\n", __func__, usb_path_state, pogo_path_state);
		if (pogo_path_state == 1)
			return 1;
	}
	return 0;
}

int get_otg_en1_state(void)
{
	int otg_en1_state=0,otg_en_state=0;
	if (pinfo == NULL)
		return 0;
	if(gpio_is_valid(pinfo->lc_otg_en1_gpio))
		otg_en1_state = gpio_get_value(pinfo->lc_otg_en1_gpio);

	if(gpio_is_valid(pinfo->lc_otg_en_gpio))
		otg_en_state = gpio_get_value(pinfo->lc_otg_en_gpio);
	pr_info("%s: otg_en1=%d,otg_en=%d\n", __func__, otg_en1_state,otg_en_state);
	if (otg_en1_state == 1)
		return 1;
	return 0;
}
...
int is_usb_in(void)
{
	...
	if(gpio_is_valid(pinfo->lc_pogo_int_gpio))
		pogo_path_state = gpio_get_value(pinfo->lc_pogo_int_gpio) ? 0:1;
	pr_info("%s: usb_int_pe=%d,pogo_int_pe=%d\n", __func__, usb_path_state,pogo_path_state);
	get_otg_en1_state();
	if (usb_path_state == 1)
		return 1;
	return 0;
...
#ifndef CONFIG_CHARGE_MACRO_P530
bool is_recheck_charge_type_status(void)
{
	if (pinfo == NULL)
		return false;
	pr_err("%s: is_charge_type_float=%d,recheck_count=%d\n",__func__,pinfo->is_charge_type_float,pinfo->recheck_count);
	if(pinfo->is_charge_type_float && (pinfo->recheck_count <= 2))
		return true;

	return false;
}
void recheck_charge_type_worker(struct work_struct *work)
{
	struct charger_manager *info = container_of(work, struct charger_manager,
					      recheck_charge_type_work.work);
	int ret = 0;
	pr_err("%s: is_charge_type_float=%d,recheck_count=%d\n",__func__,info->is_charge_type_float,info->recheck_count);
	if(info->is_charge_type_float){
		ret = charger_dev_enable_chg_type_det(info->chg1_dev, 0);
		if (ret < 0) {
			chr_err("%s: disable chgdet fail\n", __func__);
			return;
		}
		msleep(200);
		ret = charger_dev_enable_chg_type_det(info->chg1_dev, 1);
		if (ret < 0) {
...
void recheck_charge_type_func(struct charger_manager *info)
{
	unsigned int delay = msecs_to_jiffies(3000);
	if(info->chr_type == NONSTANDARD_CHARGER)
	{
		info->is_charge_type_float = true;
		pr_err("%s: is_charge_type_float=%d,recheck_count=%d\n",__func__,info->is_charge_type_float,info->recheck_count);
		if(info->recheck_count < 2)
		{
			if(get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT)
				delay = msecs_to_jiffies(2000);
			schedule_delayed_work(&info->recheck_charge_type_work,delay);
			info->recheck_count ++;
		}
		else
		{
			info->recheck_count = 4; //recheck  finish, chr_type is float.
		}
	}else
	{
		info->is_charge_type_float = false;
		info->recheck_count = 0;
	}
}
#endif
...
static int mtk_charger_plug_out(struct charger_manager *info)
{
	...
	pdata2->disable_charging_count = 0;
        ...
        is_batt_protect_below_40 = false;
        is_batt_protect_40_to_60 = false;
        is_batt_protect_above_60 = false;
#ifndef CONFIG_CHARGE_MACRO_P530
	cancel_delayed_work_sync(&info->recheck_charge_type_work);
...
bool set_chg_soc_state(struct charger_manager *info)
{
	union power_supply_propval val;
	int ret = 0,capacity = 0;

       ret = get_prop_from_bq27541(POWER_SUPPLY_PROP_CAPACITY, &val);
	 if (ret < 0)
	 	capacity = 50;
        else
	       capacity = val.intval;
	/*set charging state*/
	 if   ((capacity < 40) && (!is_batt_protect_below_40)) {
	 	is_batt_protect_below_40 = true;
		is_batt_protect_40_to_60 = false;
		is_batt_protect_above_60 = false;
		mtk_chaging_enable_write(true);
	} else if ((capacity <= 60) && (!is_batt_protect_below_40) && (!is_batt_protect_40_to_60))  {
	      is_batt_protect_40_to_60 = true;
	      is_batt_protect_above_60 = false;
		mtk_chaging_enable_write(false);
#ifndef CONFIG_CHARGE_MACRO_P530
		lc_charger_manager_enable_power_path(true);
#else
		charger_dev_enable_powerpath(info->chg1_dev, true);
#endif
	} else if ((capacity >= 60) && (!is_batt_protect_above_60) && (!is_batt_protect_40_to_60)) {
	       is_batt_protect_above_60 = true;
		is_batt_protect_below_40 = false;
		mtk_chaging_enable_write(false);
	}
	chr_err("cap=%d,is_batt_protect_below_40=%d,is_batt_protect_40_to_60=%d,is_batt_protect_above_60 =%d\n",capacity,is_batt_protect_below_40,is_batt_protect_40_to_60,is_batt_protect_above_60);
	return is_batt_protect_below_40;
}
...
static void charger_check_status(struct charger_manager *info)
{
	...
	static bool is_batt_protect_enabled = false;
	...
	if (true == g_batt_protect_mode) {
		soc_charging = set_chg_soc_state(info);
		if(!is_batt_protect_enabled)
			is_batt_protect_enabled = true;
	}
	else if (is_batt_protect_enabled) {
		is_batt_protect_enabled =false;
		is_batt_protect_40_to_60 =false;
		is_batt_protect_above_60 =false;
		if (is_batt_protect_below_40)
			is_batt_protect_below_40 = false;
		else
		mtk_chaging_enable_write(true);
	}
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d soc_chg:%d\n",
...
#if defined(CONFIG_TARGET_PROJECT_P11)
void lc_get_chargecurrent(void)
{
	int ret;
	struct power_supply *psy;
	union power_supply_propval val = {0,};
	psy = power_supply_get_by_name("bq27541");
	if (!psy)
		return;
	ret = power_supply_get_property(psy,
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		charging_pe = mtk_is_pep_series_connect(info);
...
#endif
		#if defined(CONFIG_TARGET_PROJECT_P11)
		lc_get_chargecurrent();
		#endif
		is_charger_on = mtk_is_charger_on(info);
#ifndef CONFIG_CHARGE_MACRO_P530
		if (IS_ERR_OR_NULL(mtkfb_lcm_name)) {
			chr_err("csy-nvt-mtkfb_lcm_name null!");
			} else {
				if (strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_boe_p522_drv") == 0) {
				...
				}else if(strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_inx_p522_drv") == 0) {
					if(is_charger_on) {
						 nvt_set_charger(1);//charger on
					 } else {
						nvt_set_charger(0);//charger off
					...
					//chr_err("csy-nvt-TP info: [Vendor]himax [IC]hx83102e");
				}
			}
#else
		if (IS_ERR_OR_NULL(mtkfb_lcm_name)) {
			chr_err("mtkfb_lcm_name null!");
		} else {
			if (strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_boe_drv") == 0) {
				chr_err("TP info: [Vendor]novatek [IC]nt36523w");
				if (is_charger_on) {
					chr_err("nt36523w charger on\n");
					nvt_set_charger(1);//charger on
				} else {
					chr_err("nt36523w charger off\n");
					nvt_set_charger(0);//charger off
				}
			} else {
				chr_err("TP info: [Vendor]himax [IC]hx83102e");
			}
		}
#endif
...
#if defined(CONFIG_TARGET_PROJECT_P11)
int lc_otg_enable(int value)
{
	int ret = 0;
	if (pinfo == NULL)
		return 0;
	if(gpio_is_valid(pinfo->lc_otg_en_gpio))
		ret = gpio_direction_output(pinfo->lc_otg_en_gpio, value);
	#ifdef CONFIG_POGO_SUPPORT
	pinfo->otg_in = value;
	#endif
	if(ret)
	{
		chr_err("%s: set lc_otg_en_gpio failed\n", __func__);
				...
				struct device *dev)
{
	...
	int ret = 0;
	#endif
       int power_off_charging = is_kernel_power_off_charging();
	...
	if (strcmp(info->algorithm_name, "DualSwitchCharging") == 0) {
		chr_err("found DualSwitchCharging\n");
	...
	if(power_off_charging){
		if (of_property_read_u32(np, "power_off_ac_charger_current", &val) >= 0) {
			info->data.ac_charger_current = val;
		} else if (of_property_read_u32(np, "ac_charger_current", &val) >= 0) {
			info->data.ac_charger_current = val;
		} else {
	...
	if(power_off_charging){
		if (of_property_read_u32(np, "power_off_ac_charger_input_current", &val) >= 0)
			info->data.ac_charger_input_current = val;
		else if (of_property_read_u32(np, "ac_charger_input_current", &val) >= 0)
			info->data.ac_charger_input_current = val;
		else {
	...
	if(power_off_charging){
		if (of_property_read_u32(np, "power_off_ta_ac_charger_current", &val) >= 0)
			info->data.ta_ac_charger_current = val;
		else if (of_property_read_u32(np, "ta_ac_charger_current", &val) >= 0)
			info->data.ta_ac_charger_current = val;
		else {
	...
	if(power_off_charging){
		if (of_property_read_u32(np, "power_off_ta_ac_12v_input_current", &val) >= 0)
			info->data.ta_ac_12v_input_current = val;
		else  if (of_property_read_u32(np, "ta_ac_12v_input_current", &val) >= 0)
			info->data.ta_ac_12v_input_current = val;
		else {
		...
		if (of_property_read_u32(np, "power_off_ta_ac_9v_input_current", &val) >= 0)
			info->data.ta_ac_9v_input_current = val;
		else if (of_property_read_u32(np, "ta_ac_9v_input_current", &val) >= 0)
			info->data.ta_ac_9v_input_current = val;
		else {
		...
		if (of_property_read_u32(np, "power_off_ta_ac_7v_input_current", &val) >= 0)
			info->data.ta_ac_7v_input_current = val;
		else  if (of_property_read_u32(np, "ta_ac_7v_input_current", &val) >= 0)
			info->data.ta_ac_7v_input_current = val;
		else {
...
#ifdef CONFIG_POGO_SUPPORT
	info->lc_usb_int_gpio = of_get_named_gpio(dev->of_node, "usb_int_gpio", 0);
	
	if(info->lc_usb_int_gpio < 0)
		chr_err("get usb int gpio failed\n");
	
	info->lc_pogo_int_gpio = of_get_named_gpio(dev->of_node, "pogo_int_gpio", 0);
	
	if(info->lc_pogo_int_gpio < 0)
		chr_err("get pogo int gpio failed\n");


	ret = gpio_request(info->lc_usb_int_gpio, "usb_int_gpio");	
	if(ret)
	{
		chr_err("%s: request usb_int_gpio failed\n", __func__);

	}	

	ret = gpio_request(info->lc_pogo_int_gpio, "pogo_int_gpio");	
	if(ret)
	{
		chr_err("%s: request pogo_int_gpio failed\n", __func__);

	}	
	info->lc_otg_en1_gpio = of_get_named_gpio(dev->of_node, "otg_en1_gpio", 0);

	if(info->lc_otg_en1_gpio < 0)
		chr_err("get otg en gpio failed\n");

	ret = gpio_request(info->lc_otg_en1_gpio, "otg_en1_gpio");

	if(ret)
	{
		chr_err("%s: request otg_en1_gpio failed\n", __func__);
	}

	if(gpio_is_valid(info->lc_otg_en1_gpio))
		ret = gpio_direction_output(info->lc_otg_en1_gpio, 1);

	if(ret)
	{
		chr_err("%s: set lc_otg_en1_gpio failed\n", __func__);
	}
#endif	
#if defined(CONFIG_TARGET_PROJECT_P11)
	info->lc_otg_en_gpio = of_get_named_gpio(dev->of_node, "otg_en_gpio", 0);

	if(info->lc_otg_en_gpio < 0)
		chr_err("get otg en gpio failed\n");

	ret = gpio_request(info->lc_otg_en_gpio, "otg_en_gpio");

	if(ret)
	{
		chr_err("%s: request otg_en_gpio failed\n", __func__);
	}

	if(gpio_is_valid(info->lc_otg_en_gpio))
		ret = gpio_direction_output(info->lc_otg_en_gpio, 0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (mtk_is_pep_series_connect(pinfo) == true)
	{
		is_ta_detected = 1;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	bat_temp = force_get_tbat(true);
	pr_debug("%s: bat_temp:%d\n", bat_temp);

	return sprintf(buf, "%d\n", bat_temp);
}

static DEVICE_ATTR(battery_temperature, 0444, show_battery_temperature, NULL);

static ssize_t show_BatteryPresent(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(BatteryPresent, 0444, show_BatteryPresent, NULL);
...
void mtk_chaging_enable_write(int en)
{
	...
	{
#ifndef CONFIG_CHARGE_MACRO_P530
	      lc_charger_manager_enable_power_path(en);
#else
	      charger_dev_enable_powerpath(pinfo->chg1_dev, en);
#endif
		if (en == 0) {
...
#ifndef CONFIG_CHARGE_MACRO_P530
void lc_chaging_enable_write(int en)
{
	if (mt_get_charger_type() == CHARGER_UNKNOWN)
		return;
	...
	else
	{
		...
		lc_charger_manager_enable_power_path(true);
		pr_err("lct charging_enabled=%d",en);
	}
}

void lc_input_suspend_write(int en)
{
       int enabled = !en;
	...
	{
		pr_err("lct input_suspend=%d,enabled=%d",en,enabled);
	      lc_charger_manager_enable_power_path(enabled);
		if (enabled == 0) {
			pinfo->cmd_discharging = true;
			charger_dev_enable(pinfo->chg1_dev, false);
			charger_manager_notifier(pinfo,
...
#endif
void batt_protect_charging_enable(bool en)
{
	union power_supply_propval val;
	int ret = 0;
	...
	ret = get_prop_from_bq27541(POWER_SUPPLY_PROP_CAPACITY, &val);
	 if (ret < 0) {
		...
		if ((en == 1) &&(val.intval >= 60)) {
#ifndef CONFIG_CHARGE_MACRO_P530
			lc_charger_manager_enable_power_path(false);
#else
			...
			charger_manager_notifier(pinfo,
						CHARGER_NOTIFY_STOP_CHARGING);
			chr_err("batt_protect_discharging\n");
...
#ifdef CONFIG_POGO_SUPPORT
#define  POGO_WAKELOCK_HOLD_TIME 3000 /* in ms */
#define  LC_VALID_VBUS  (4500)   /* in mv */
void lc_set_keyboard_status(int status){
	if (pinfo == NULL)
		return;
	pinfo->keyboard_status = status;
	pr_err("%s:keyboard_status=%d\n",__func__,pinfo->keyboard_status);
}
static int pogo_charger_switch_update(struct  charger_manager *info)
{
	int usb_path_state = 0,pogo_path_state =0;
	int lc_vbus_value = 0;
	if(pinfo->keyboard_status != 0){
		if(pinfo->keyboard_status == 2){
			lc_set_keyboard_status(0);
			info->usb_in = 0;
			info->pogo_in = 0;
		}
		return 0;
	}
	if(gpio_is_valid(info->lc_usb_int_gpio))
		usb_path_state = gpio_get_value(info->lc_usb_int_gpio) ? 0:1;
	if(gpio_is_valid(info->lc_pogo_int_gpio))
		pogo_path_state = gpio_get_value(info->lc_pogo_int_gpio) ? 0:1;
	pr_info("%s: usb_int %d, pogo_int %d,is_pogo_invalid %d\n", __func__,
		usb_path_state, pogo_path_state,info->is_pogo_invalid);
	if(pinfo->otg_in == 1 && pogo_path_state == 1){
		gpio_direction_output(info->lc_otg_en_gpio, 1);
		msleep(50);
		gpio_direction_output(info->lc_otg_en1_gpio, 0);
		msleep(10);
	}else if(pinfo->otg_in == 1 && pogo_path_state == 0){
		gpio_direction_output(info->lc_otg_en_gpio, 1);
		msleep(50);
		gpio_direction_output(info->lc_otg_en1_gpio, 1);
		msleep(10);
	}else if(usb_path_state == 0 && pogo_path_state == 1){
		// only pogo insert
		gpio_direction_output(info->lc_otg_en_gpio, 0);
		msleep(50);
		gpio_direction_output(info->lc_otg_en1_gpio, 0);
		msleep(10);
	}else {
	...
	lc_vbus_value = charger_get_vbus();
	if((lc_vbus_value < LC_VALID_VBUS) && (pogo_path_state == 1)){
		pr_info("vbus is invalid\n");
		if(lc_vbus_value >= 0){
			pr_info("It is not pogo charging mode,return\n");
			info->is_pogo_invalid = 1;
			return 0;
		}
	}else if((info->is_pogo_invalid == 1 && pogo_path_state == 0) ||
		(info->pogo_in == 0 && (lc_vbus_value < LC_VALID_VBUS))){
		info->is_pogo_invalid = 0;
		pr_info("It is not pogo charging mode,return\n");
		return 0;
	}else{
		info->is_pogo_invalid = 0;
	}
	pr_info("%s: usb_int_back %d, pogo_int_back %d\n", __func__, info->usb_in, info->pogo_in);
	if (!pogo_path_state && !info->pogo_in) //schedule work when system power on and usb connect,return
		return 0;
	if ((!usb_path_state || !!pinfo->otg_in) && !!pogo_path_state ) {
		pr_err("pogo charger plug in");
		charger_manager_enable_power_path(info->chg1_consumer,
					MAIN_CHARGER, true);
		info->is_pogo_enable_charging = 1;

	}else if (!pogo_path_state && !!info->is_pogo_enable_charging){
		pr_err("pogo charger plug out");
		info->is_pogo_invalid = 0;
		if(!usb_path_state)
			charger_manager_enable_power_path(info->chg1_consumer,
					MAIN_CHARGER, false);
		info->is_pogo_enable_charging = 0;
	}else{
		if(!pogo_path_state && info->pogo_in){
			pr_err("pogo plug out,update charge type\n");
			info->pogo_in = pogo_path_state;
			info->usb_in = usb_path_state;
			mt_get_charger_type();
			_wake_up_charger(info);
		}else{
			info->usb_in = usb_path_state;
...
void pogo_int_worker(struct work_struct *work)
{
	struct charger_manager *info = container_of(work, struct charger_manager,
					      pogo_int_work.work);
	pogo_charger_switch_update(info);
}
irqreturn_t pogo_charger_switch_handler(int irq, void *data)
{
	struct charger_manager *info = data;
	__pm_wakeup_event(&info->lc_pogo_ws, POGO_WAKELOCK_HOLD_TIME);
	cancel_delayed_work(&info->pogo_int_work);
	schedule_delayed_work(&info->pogo_int_work,msecs_to_jiffies(1000));
	return IRQ_HANDLED;
}
...
irqreturn_t usb_charger_switch_handler(int irq, void *data)
{
	...
	if(gpio_is_valid(info->lc_pogo_int_gpio))
		pogo_path_state = gpio_get_value(info->lc_pogo_int_gpio) ? 0:1;
	pr_info("%s: usb_int %d, pogo_int %d,otg_in %d\n", __func__, usb_path_state, pogo_path_state,pinfo->otg_in);
	if(pinfo->keyboard_status != 0 || info->is_pogo_invalid ==1)
		pogo_path_state = 0;
	if(pinfo->otg_in == 1 && pogo_path_state == 1){//otg,pogo
			gpio_direction_output(info->lc_otg_en_gpio, 1);
			msleep(50);
			gpio_direction_output(info->lc_otg_en1_gpio, 0);
			msleep(10);
	}else if(pinfo->otg_in == 1 && pogo_path_state == 0){//otg only
			gpio_direction_output(info->lc_otg_en_gpio, 1);
			msleep(50);
			gpio_direction_output(info->lc_otg_en1_gpio, 1);
			msleep(10);
	}else if (usb_path_state == 0 && pogo_path_state == 1) {//usb out,pogo
			gpio_direction_output(info->lc_otg_en1_gpio, 1);
			gpio_direction_output(info->lc_otg_en_gpio, 0);
			__pm_wakeup_event(&info->lc_pogo_ws, POGO_WAKELOCK_HOLD_TIME);
			//cancel_delayed_work(&info->pogo_int_work);
			schedule_delayed_work(&info->pogo_int_work,msecs_to_jiffies(3000));
	}else {
			gpio_direction_output(info->lc_otg_en1_gpio, 1);
			msleep(50);
			gpio_direction_output(info->lc_otg_en_gpio, 0);
...
static int pogo_int_init(struct charger_manager *info)
{
	int rc = 0;
	rc = devm_request_threaded_irq(&info->pdev->dev,
			gpio_to_irq(info->lc_pogo_int_gpio), NULL,
	...
	if (rc < 0) {
		pr_err("%s: Couldn't request pogo_int irq %d\n", __func__,
			gpio_to_irq(info->lc_pogo_int_gpio));
		return rc;
	}

	rc = devm_request_threaded_irq(&info->pdev->dev,
			gpio_to_irq(info->lc_usb_int_gpio), NULL,
	...
	if (rc < 0) {
		pr_err("%s: Couldn't request usb_int irq %d\n", __func__,
			gpio_to_irq(info->lc_usb_int_gpio));
		return rc;
	}
	enable_irq_wake(gpio_to_irq(info->lc_pogo_int_gpio));
	enable_irq_wake(gpio_to_irq(info->lc_usb_int_gpio));
	wakeup_source_init(&info->lc_pogo_ws, "lct-pogo-chg");
...
#ifdef CONFIG_POGO_SUPPORT
	pogo_int_init(info);
	INIT_DELAYED_WORK(&info->pogo_int_work, pogo_int_worker);
#endif
#ifndef CONFIG_CHARGE_MACRO_P530
	INIT_DELAYED_WORK(&info->recheck_charge_type_work, recheck_charge_type_worker);
#endif
...
static int mtk_charger_remove(struct platform_device *dev)
{
	...
	#ifdef CONFIG_POGO_SUPPORT
	wakeup_source_trash(&info->lc_pogo_ws);
	#endif
	return 0;
}
