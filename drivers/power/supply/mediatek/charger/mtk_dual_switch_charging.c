...
int batt_maintenance_mode = 0; //0: 4430mV; 1: 4380mV; 2: 4230mV
...
static void set_jeita_charge_current(struct charger_manager *info,struct charger_data *pdata)
{
	struct charger_data *pdata2;
	pdata2 = &info->chg2_data;
	pr_info("%s: info->sw_jeita.sm =%d,charging_current_limit=%d\n",__func__,info->sw_jeita.sm,pdata->charging_current_limit);
	...
	if (pdata2->input_current_limit != 0 &&
...
dual_swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_pe40_get_is_connect(info)) {
		...
		if (!dual_swchg_check_pd_leave(info) && info->chr_type != NONSTANDARD_CHARGER) {
			/* Slave charger may not have input current control */
			if(vbus == 5000){
				pdata2->input_current_limit = cur * 1000/2;
				pdata->input_current_limit = cur * 1000/2;
			}
			else{
				pdata2->input_current_limit = info->data.ac_charger_input_current/2;
				pdata->input_current_limit = info->data.ac_charger_input_current/2;
			}
			switch (swchgalg->state) {
			case CHR_CC:
				pdata->charging_current_limit
	...
	if (!!is_pogo_charging()) {
			printk("%s,It is pogo charging.The current is same with DCP\n",__func__);
			pdata->input_current_limit = 2000000;
			pdata->charging_current_limit = 2000000;
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			#endif
			set_jeita_charge_current(info,pdata);
			if (pdata2->input_current_limit != 0 &&
			    pdata2->charging_current_limit != 0) {//dual charge need set slaver charger
			    set_jeita_charge_current(info,pdata2);
			}
		}
	}
	/*
	...
	#if 0 //def CONFIG_POGO_SUPPORT
	else if (!!is_pogo_charging()){
		if (chg2_chip_enabled) {
			charger_dev_set_input_current(info->chg2_dev,
				pdata2->input_current_limit);
			charger_dev_set_charging_current(info->chg2_dev,
...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	bool chg2_chip_enabled = false;
	int rc = 0,g_sw_jeita_cv = 0;
	static int charge_elapsed_time_month = 0;
	static int charge_cycle_count = 0;
	union power_supply_propval pval = {0, };
	struct power_supply *psy = power_supply_get_by_name("bq27541");

	if (!psy){
		pr_err("%s bq27541 psy is null!\n",__func__);
	} else {
		...
		rc = power_supply_get_property(psy,POWER_SUPPLY_PROP_ELAPSED_TIME_MONTH,&pval);
		if(charge_elapsed_time_month != pval.intval)
			charge_elapsed_time_month = pval.intval;
        }
	...
	if (info->enable_sw_jeita)
		if (info->sw_jeita.cv != 0) {
			if((charge_cycle_count > 410) ||(charge_elapsed_time_month > 50)) {
				g_sw_jeita_cv = info->sw_jeita.cv - 230000;
				if(batt_maintenance_mode != 3)
					batt_maintenance_mode = 3;
			} else if ((charge_cycle_count > 400 && charge_cycle_count <= 410) ||
				(charge_elapsed_time_month > 48 && charge_elapsed_time_month <= 50)) {
				g_sw_jeita_cv = info->sw_jeita.cv - 130000;
				if(batt_maintenance_mode != 2)
					batt_maintenance_mode = 2;
			} else if ((charge_cycle_count > 150 && charge_cycle_count <= 400) ||
				(charge_elapsed_time_month > 18 && charge_elapsed_time_month <= 48)) {
				g_sw_jeita_cv = info->sw_jeita.cv - 50000;
				if(batt_maintenance_mode != 1)
					batt_maintenance_mode = 1;
			} else {
				g_sw_jeita_cv = info->sw_jeita.cv;
				if(batt_maintenance_mode != 0)
					batt_maintenance_mode = 0;
			}
			chr_err("%s,g_sw_jeita_cv=%d,cycle:%d,charge_elapsed_time_month=%d\n",__func__,g_sw_jeita_cv,charge_cycle_count,charge_elapsed_time_month);
	...
	/* dynamic cv*/
	if((charge_cycle_count > 410) ||(charge_elapsed_time_month > 50)) {
		constant_voltage =  info->data.battery_cv- 230000;
		if(batt_maintenance_mode != 3)
			batt_maintenance_mode = 3;
	} else if ((charge_cycle_count > 400 && charge_cycle_count <= 410) ||
		(charge_elapsed_time_month > 48 && charge_elapsed_time_month <= 50)) {
		constant_voltage =  info->data.battery_cv - 130000;
		if(batt_maintenance_mode != 2)
			batt_maintenance_mode = 2;
	} else if ((charge_cycle_count > 150 && charge_cycle_count <= 400) ||
		(charge_elapsed_time_month > 18 && charge_elapsed_time_month <= 48)) {
		constant_voltage =  info->data.battery_cv - 50000;
		if(batt_maintenance_mode != 1)
			batt_maintenance_mode = 1;
	} else {
	...
	chr_err("%s,constant_voltage=%d,cycle:%d,charge_elapsed_time_month=%d\n",__func__,constant_voltage,charge_cycle_count,charge_elapsed_time_month);
...
static void dual_swchg_turn_on_charging(struct charger_manager *info)
{
	...
	if (chg2_enable == true) {
		if ((mtk_pe20_get_is_enable(info) &&
			...
			charger_dev_set_eoc_current(info->chg1_dev, 350000);
	...
	if (chg2_enable) {
			msleep(300);
			charger_dev_set_input_current(info->chg2_dev,
				info->chg2_data.input_current_limit);
			charger_dev_set_charging_current(info->chg2_dev,
				info->chg2_data.charging_current_limit);
	}
	#endif
	if (info->data.parallel_vbus) {
		if (!chg2_enable) {
