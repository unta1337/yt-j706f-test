...
struct charger_manager {
	...
	int keyboard_status;
	int is_pogo_invalid;
	int is_pogo_enable_charging;
	struct delayed_work pogo_int_work;
	struct wakeup_source	lc_pogo_ws;
#endif
#if defined(CONFIG_TARGET_PROJECT_P11)
	int lc_otg_en_gpio;
#endif
#ifndef CONFIG_CHARGE_MACRO_P530
	struct delayed_work recheck_charge_type_work;
	bool is_charge_type_float;
	int recheck_count;
#endif
...
extern void notify_adapter_event(enum adapter_type type, enum adapter_event evt,
	void *val);
#ifdef CONFIG_POGO_SUPPORT
extern int is_pogo_charging(void);
extern int is_pogo_connect(void);
extern int is_usb_in(void);
extern void disable_pogo(void);
#endif
