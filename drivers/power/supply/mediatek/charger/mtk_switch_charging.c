...
int standard_charger_500 = 0;
#ifdef CONFIG_CHARGE_MACRO_P530
...
if (!!is_pogo_charging()) {
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			if ((info->sw_jeita.sm == TEMP_T0_TO_T1) || (info->sw_jeita.sm == TEMP_T1_TO_T2)) {
				//pdata->input_current_limit = 500000;
				pdata->charging_current_limit = 1500000;
			}
			if (info->sw_jeita.sm == TEMP_BELOW_T0) {
					if (pdata->input_current_limit >= 2100000)
					...
					if (is_kernel_power_off_charging()) {
						if (pdata->input_current_limit >= 2100000)
							pdata->input_current_limit = 2100000;
						if (pdata->charging_current_limit >= 4100000)
							pdata->charging_current_limit = 4100000;
					} else {
						if (pdata->input_current_limit >= 2100000)
							pdata->input_current_limit = 2100000;
						if (pdata->charging_current_limit >= 3700000)
							pdata->charging_current_limit = 3700000;
					}
