...
/*
 * DW9763QTECHAF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "DW9763QTECHAF_DRV"
...
static int s4DW9763QTECHAF_ReadReg(unsigned short *a_pu2Result)
{
...
/* ////////////////////////////////////////////////////////////// */
long DW9763QTECHAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int DW9763QTECHAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	unsigned long temp;
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
		for (temp = g_u4CurrPosition; temp >= 20; temp -= 20) {
			s4AF_WriteReg(temp);
			msleep(15);
		}
	}
...
int DW9763QTECHAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int DW9763QTECHAF_GetFileName(unsigned char *pFileName)
{
