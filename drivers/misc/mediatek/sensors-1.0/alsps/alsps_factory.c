					 ...
					 unsigned long arg)
{
	...
	case ALSPS_GET_PS_RAW_DATA:
		if (alsps_factory.fops != NULL &&
			...
			err = alsps_factory.fops->ps_get_raw_data(alsps_data);
			if (err < 0) {
				pr_err(
					"ALSPS_GET_PS_RAW_DATA read data fail!\n");
				return -EINVAL;
			}
			if (copy_to_user(ptr, &alsps_data, sizeof(alsps_data)))
				return -EFAULT;
		} else {
	...
	case ALSPS_GET_ALS_RAW_DATA:
		if (alsps_factory.fops != NULL &&
			...
			err = alsps_factory.fops->als_get_raw_data(alsps_data);
			if (err < 0) {
				pr_err(
...
#ifdef CONFIG_SENSOR_PARAM_TO_SCP
                if (copy_from_user(&ps_cali_order, ptr, sizeof(ps_cali_order)))
                        return -EFAULT;
                pr_info("ALSPS_PS_ENABLE_CALI arg = %d\n", ps_cali_order);
                if (sensor_para != NULL)
                        sensor_para->ps_para.cali_order = ps_cali_order;
#endif
		if (alsps_factory.fops != NULL &&
