...
}   /*  s5k4h7yxsubMIPI_Capture_Setting  */
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				module_id = get_txd_module_id();
				if(module_id != 0x0b) {
					printk("Read TXD module id fail,module id = 0x%x\n",module_id);
...
UINT32 S5K4H7TXD_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	s5k4h7txd_MIPI_RAW_SensorInit	*/
