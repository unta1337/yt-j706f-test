...
{{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0000, 0000, 1632, 1224, 0, 0, 3264, 2448}, // Preview
 ...
 { 3264, 2448, 0, 0, 3264, 2448, 640, 480, 0000, 0000, 640, 480, 0, 0, 640, 480 },  //hs video
...
}   /*  s5k4h7yxlceMIPI_Capture_Setting  */
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				module_id = get_lce_module_id();
				if(module_id != 0x0A) {
					LOG_INF("Read LCE module id failed,module id = 0x%x\n",module_id);
...
*/
...
#ifdef VENDOR_EDIT
	 /*  add for otp*/
	otp_flag = S5K4H7YXLCE_otp_update();
	if (otp_flag)
	...
	spin_unlock(&imgsensor_drv_lock);
	s5k4h7_check = 4; // s5k4h7yxlce 487d
	return ERROR_NONE;
}	/*	open  */
...
UINT32 S5K4H7YXLCE_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	s5k4h7yxlce_MIPI_RAW_SensorInit	*/
