...
#define LOG_1 LOG_INF("s5k4h7yxsub,MIPI 2LANE\n")
#define LOG_2 \
...
kal_uint8 s5k4h7yxsub_2a_data[33];

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static MUINT32 g_sync_mode = SENSOR_NO_SYNC_MODE;
//static int first_flag = 1;
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YXSUB_SENSOR_ID,  //0x487B
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(module_id != 0x07) {
					printk("Read OFLIM module id failed,module id = 0x%x\n",module_id);
