...
{
	int page;
	memset(&s5k4h7yxsub_otp_info, 0, sizeof(OTP));

	s5k4h7yxsub_otp_info.lsc_infoflag = s5k4h7yxsub_selective_read_region_8(0, 0x0A3D);  //page 0

	if (s5k4h7yxsub_otp_info.lsc_infoflag == 0x01) {
		s5k4h7yxsub_otp_info.lsc_offset = 0;
		s5k4h7yxsub_otp_info.lsc_group = 1;
		s5k4h7yxsub_otp_info.lsc_checksum = s5k4h7yxsub_selective_read_region_8(24, 0x0A06);  //page 24
		...
		printk("S5K4H7YXSUB OTP read data fail lsc empty!!!\n");
		goto error;
	}

	for (page = 21;page <= 23 ;page++){
		s5k4h7yxsub_otp_info.awb_infoflag = s5k4h7yxsub_selective_read_region_8(page, 0x0A04);
	...
	s5k4h7yxsub_otp_info.module_integrator_id = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A05);
	s5k4h7yxsub_otp_info.awb_checksum = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A2D);

	s5k4h7_ofilm_awb_r_H = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A1F);
	s5k4h7_ofilm_awb_r_L = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A20);
	s5k4h7_ofilm_awb_b_H = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A25);
	s5k4h7_ofilm_awb_b_L = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A26);
	s5k4h7_ofilm_awb_gr_H = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A21);
	s5k4h7_ofilm_awb_gr_L = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A22);
	s5k4h7_ofilm_awb_gb_H = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A23);
	s5k4h7_ofilm_awb_gb_L = s5k4h7yxsub_selective_read_region_8(s5k4h7yxsub_otp_info.awb_page, 0x0A24);
...
*********/
unsigned int s5k4h7yxsub_checksum_lsc_flag(unsigned int sum_start, unsigned int sum_end, unsigned char *pdata)
{
	int i = 0;
	unsigned int start;
...
bool S5K4H7YXSUB_checksum_awb(void)
{
	...
	printk("checksum_awb = %d,s5k4h7yxsub_otp_info.checksum_awb = %d",checksum_awb,s5k4h7yxsub_otp_info.awb_checksum);

	if (checksum_awb != s5k4h7yxsub_otp_info.awb_checksum) {
		s5k4h7_ofilm_awb_r_H = 0;
		s5k4h7_ofilm_awb_r_L = 0;
		s5k4h7_ofilm_awb_b_H = 0;
		s5k4h7_ofilm_awb_b_L = 0;
		s5k4h7_ofilm_awb_gr_H = 0;
		s5k4h7_ofilm_awb_gr_L = 0;
		s5k4h7_ofilm_awb_gb_H = 0;
		s5k4h7_ofilm_awb_gb_L = 0;
		printk("S5K4H7YXSUB OTP checksum awb flag sum fail!!!");
		bresult &= 0;
	}
	return  bresult;
}
...
bool S5K4H7YXSUB_checksum_lsc(void)
{
	...
	if (s5k4h7yxsub_otp_info.lsc_group == 1) {
		for (page_start = 1, page_end = 6; page_start <= page_end; page_start++) {
			bresult &= s5k4h7yxsub_read_page(page_start, page_start, data_p[page_start-1]);
			if (page_start == 6) {
				checksum_lsc += s5k4h7yxsub_checksum_lsc_flag(0x0A04, 0x0A2B, data_p[page_start-1]);
				continue;
			}
			checksum_lsc += s5k4h7yxsub_checksum_lsc_flag(0x0A04, 0x0A43, data_p[page_start-1]);
		}
	} else if (s5k4h7yxsub_otp_info.lsc_group == 2) {
		for (page_start = 6, page_end = 12; page_start <= page_end; page_start++) {
			bresult &= s5k4h7yxsub_read_page(page_start, page_start, data_p[page_start-1]);
			if (page_start == 6) {
				checksum_lsc += s5k4h7yxsub_checksum_lsc_flag(0x0A2C, 0x0A43, data_p[page_start-1]);
				continue;
			} else if (page_start < 12) {
				checksum_lsc += s5k4h7yxsub_checksum_lsc_flag(0x0A04, 0x0A43, data_p[page_start-1]);
			} else {
				checksum_lsc += s5k4h7yxsub_checksum_lsc_flag(0x0A04, 0x0A13, data_p[page_start-1]);
			}
		}
	}
	checksum_lsc = (checksum_lsc) % 255 + 1;
	printk("checksum_lsc = %d,s5k4h7yxsub_otp_info.checksum_lsc = %d",checksum_lsc,s5k4h7yxsub_otp_info.lsc_checksum);

	if (checksum_lsc == s5k4h7yxsub_otp_info.lsc_checksum) {
		s5k4h7yxsub_apply_otp_lsc();
	} else {
		printk("S5K4H7YXSUB OTP checksum lsc sum fail!!!");
		bresult &= 0;
	}
...
bool S5K4H7YXSUB_otp_update(void)
{
	...
	if(s5k4h7yxsub_otp_group_info() == -1){
		...
		if(S5K4H7YXSUB_checksum_awb() == 0 || S5K4H7YXSUB_checksum_lsc() == 0 ){
			printk("S5K4H7YXSUB OTP checksum sum fail!!!\n");
			result &= 0;
		}
		else {
			printk("S5K4H7YXSUB OTP checksumsum ok\n");
		}
	}
	return  result;
}

UINT8 get_ofilm_module_id(void)
{
	int page;
	UINT8 group_info, module_integrator_id;
	
	for (page = 21;page <= 23 ;page++){
		group_info = s5k4h7yxsub_selective_read_region_8(page, 0x0A04);
		if ((group_info>>6 & 0x03 )== 0x01){
			module_integrator_id = s5k4h7yxsub_selective_read_region_8(page, 0x0A05);			
