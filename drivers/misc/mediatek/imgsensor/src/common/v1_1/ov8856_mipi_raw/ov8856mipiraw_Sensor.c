...
extern int ov8856_awb_R;
extern int ov8856_awb_B;
extern int ov8856_awb_Gr;
extern int ov8856_awb_Gb;
extern int ov8856_awb_lsb;
...
int read_otp(void)
{
	...
	if (addr != 0) {
		...
		otp_data.module_integrator_id = read_cmos_sensor(addr);
		otp_data.sensor_id = read_cmos_sensor(addr + 1);

		for (i = 0; i < 10; i++) {
			otp_data.awbc[i] = read_cmos_sensor(addr + i);
			checksum2 += otp_data.awbc[i];
		}
		...
		if(addr == 0x7011)
			otp_data.checksum =read_cmos_sensor(0x7025);
		else
			otp_data.checksum =read_cmos_sensor(0x7026);
		...
		if (otp_data.checksum == checksum2) {
			...
			ov8856_awb_R = read_cmos_sensor(addr + 2);
			ov8856_awb_Gr = read_cmos_sensor(addr + 3);
			ov8856_awb_Gb = read_cmos_sensor(addr + 4);
			ov8856_awb_B = read_cmos_sensor(addr + 5);
			ov8856_awb_lsb = read_cmos_sensor(addr + 6);
		}

		LOG_INF("awb,af,group id checksum = %d,checksum2 = %d\n",otp_data.checksum,checksum2);
		LOG_INF("ov8856_awb_R = %d,ov8856_awb_Gr = %d,ov8856_awb_Gb = %d,ov8856_awb_B = %d,ov8856_awb_lsb = %d,AFInf = %d,AFMac = %d,AFLsb = %d\n",ov8856_awb_R,ov8856_awb_Gr,ov8856_awb_Gb,ov8856_awb_B,ov8856_awb_lsb,ov8856_af_inf,ov8856_af_mac,ov8856_af_lsb);
	} else {
		otp_data.flag = 0x00;	/* not info and AWB in OTP */
		otp_data.module_integrator_id = 0;
		otp_data.lens_id = 0;
		otp_data.production_year = 0;
		otp_data.production_month = 0;
		otp_data.production_day = 0;
		otp_data.rg_ratio = 0;
		otp_data.bg_ratio = 0;
	}
	...
	if (addr != 0) {
		for (i = 0; i < 240; i++) {
			otp_data.lenc[i] = read_cmos_sensor(addr + i);
			checksum2 += otp_data.lenc[i];
		}
		checksum2 = (checksum2) % 255 + 1;
		otp_data.checksum = read_cmos_sensor(addr + 240);
...
int apply_otp(void)
{
	...
	if (otp_data.flag & 0x10) {
		...
		for (i = 0; i < 240; i++)
			write_cmos_sensor(0x5900 + i, otp_data.lenc[i]);
