...
#define IRQ_HOLD_TIME 5000 //ms
struct wakeup_source *pmic_irq_lock;
...
irqreturn_t key_int_handler(int irq, void *data)
{
	...
	__pm_wakeup_event(pmic_irq_lock, IRQ_HOLD_TIME);
...
void PMIC_EINT_SETTING(struct platform_device *pdev)
{
	...
	pmic_irq_lock = wakeup_source_register(&pdev->dev, "key irq wakelock");
}
