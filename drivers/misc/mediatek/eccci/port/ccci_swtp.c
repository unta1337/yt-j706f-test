...
static int swtp_switch_state(int irq, struct swtp_t *swtp)
{
	...
	if (swtp->gpio_state[i] == SWTP_EINT_PIN_PLUG_IN){
		...
		if(i == 1){
			input_report_key(swtp_ipdev, DIV_ANT_CONNECT, 1);
			input_report_key(swtp_ipdev, DIV_ANT_CONNECT, 0);
			input_sync(swtp_ipdev);
		...
		if(i == 1){
			input_report_key(swtp_ipdev, DIV_ANT_UNCONNECT, 1);
			input_report_key(swtp_ipdev, DIV_ANT_UNCONNECT, 0);
			input_sync(swtp_ipdev);
