...
int do_esd_recover = 0;
#if defined(CONFIG_TARGET_PROJECT_P522)
extern void nvt_esd_recovery(int flag);
extern void himax_esd_recovery(int flag);
#endif
...
int primary_display_esd_recovery(void)
{
	...
	/*2021.04.07 longcheer chenshiyang add for himax IC Frozen screen issue begin*/
	DISPCHECK("[ESD] TP suspend[begin]\n");
	if (strnstr(mtkfb_lcm_name, "hx83102p", strlen(mtkfb_lcm_name))) {
#if defined(CONFIG_TARGET_PROJECT_P522)
		himax_esd_recovery(TP_SUSPEND);
#endif
		DISPCHECK("[ESD] It is himax IC, do tp suspend\n");
	} else if (strnstr(mtkfb_lcm_name, "nt36523w", strlen(mtkfb_lcm_name))) {
#if defined(CONFIG_TARGET_PROJECT_P522)
		nvt_esd_recovery(TP_SUSPEND);
#endif
		DISPCHECK("[ESD] It is NVT IC, do tp suspend\n");
	} else {
		DISPCHECK("[ESD] It is not find IC, Skip\n");
	}
	DISPCHECK("[ESD] TP suspend[end]\n");
	/*2021.04.07 longcheer chenshiyang add for himax IC Frozen screen issue end*/
	...
	/*2021.04.07 longcheer chenshiyang add for himax IC Frozen screen issue begin*/
	DISPCHECK("[ESD] TP resume[begin]\n");
	if (strnstr(mtkfb_lcm_name, "hx83102p", strlen(mtkfb_lcm_name))) {
#if defined(CONFIG_TARGET_PROJECT_P522)
		himax_esd_recovery(TP_RESUME);
#endif
		DISPCHECK("[ESD] It is himax IC, do tp resume\n");
	} else if (strnstr(mtkfb_lcm_name,"nt36523w", strlen(mtkfb_lcm_name))) {
#if defined(CONFIG_TARGET_PROJECT_P522)
		nvt_esd_recovery(TP_RESUME);
#endif
		DISPCHECK("[ESD] It is NVT IC, do tp resume\n");
	} else {
		DISPCHECK("[ESD] It is not find IC, Skip\n");
	}
	DISPCHECK("[ESD] TP resume[end]\n");
	/*2021.04.07 longcheer chenshiyang add for himax IC Frozen screen issue end*/
