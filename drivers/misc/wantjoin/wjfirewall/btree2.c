...
BTREE *btree_find(PROC_CMP pCmp, BTREE *node, const BTREE_KEY num, BTREE_POS *pos)
{
	BTREE_POS i;
	BTREE *prev=NULL;
...
void restore_nodes(PROC_CMP pCmp, BTREE *pParent, BTREE *pChild)
{
	BTREE_POS i, pos;

	for (i = 0; i < pChild->count; ++ i)
	{
		btree_add_internal(pCmp, pParent, (BTREE *)NULL, (BTREE *)NULL, pChild->key[i], &pos);
	}
	for (i = 0; i <= pChild->count; ++ i)
	{
		if (pChild->ptr[i])
		{
			restore_nodes(pCmp, pParent, pChild->ptr[i]);
		}
	}
}

void btree_delete(BTREE *node, BTREE_POS pos, PROC_CMP pCmp)
{

	BTREE_POS i;
	BTREE *pChild = node->ptr[pos];
	{
		{
			-- node->count;
			...
			node->ptr[node->count] = node->ptr[node->count + 1];
		}
	}

	
	node->stored = (node->stored | BTREE_BIT(node->count)) ^ BTREE_BIT(node->count);

	if (pChild)
	{
		restore_nodes(pCmp, node, pChild);
		btree_free(pChild, NULL);
	}
}
...
void btree_free(BTREE *node, PROC_FREE pFree)
{
	BTREE_POS i;
