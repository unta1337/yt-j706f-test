		...
		unsigned long **bit, int *max)
{
	...
	case HID_UP_BUTTON:
		...
		if (field->application == HID_DG_TOUCHPAD && (usage->hid & HID_USAGE) > 1) {
			code--;
			}
		/*
	...
	if (hid->claimed & HID_CLAIMED_INPUT) {
		...
		case HID_GD_X:
			if (usage->code == ABS_MT_TOOL_X)
				...
				td->curdata.x = value;
			//dev_err(&hid->dev, "lq_keyboard:x:value:%d\n", value);
			break;
		case HID_GD_Y:
			if (usage->code == ABS_MT_TOOL_Y)
				...
				td->curdata.y = value;
			//dev_err(&hid->dev, "lq_keyboard:y:value:%d\n", value);
			break;
		case HID_DG_WIDTH:
			td->curdata.w = value;
			break;
		...
		//dev_err(&hid->dev, "lq_keyboard:touchpad:mouse key pressed!\n");
        //dev_err(&hid->dev, "lq_keyboard:touchpad:usage->code = 0x%x \n", usage->code);
        ...
        //dev_err(&hid->dev, "lq_keyboard:field->hidinput->input->name:%s\n", field->hidinput->input->name);
		if (usage->usage_index + 1 == field->report_count) {
			/* we only take into account the last report. */
			if (usage->hid == td->last_slot_field)
			{
				//dev_err(&hid->dev, "lq_keyboard:mt_complete_slot\n");
