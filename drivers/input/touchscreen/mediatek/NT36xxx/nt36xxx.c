...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	NVT_LOG("ts->gesture_enabled=%d tp_work=%d \n",ts->gesture_enabled,get_lct_tp_work_status());
	


#ifdef LCT_TP_WORK_EN
		if (get_lct_tp_work_status() && ts->gesture_enabled) {
			NVT_LOG("Touchscreen gesture_mode, Can't enable irq!");
		} else if (get_lct_tp_work_status() && !ts->gesture_enabled) {
			nvt_irq_enable(true);
		} else if (get_lct_tp_work_status()) {
			if (ts->gesture_enabled) {
				nvt_irq_enable(true);
			}
		} else {
	...
	//Check TP vendor
		...
		if (IS_ERR_OR_NULL(mtkfb_lcm_name)) {
			...
			if (strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_boe_drv") == 0) {
				NVT_LOG("TP info: [Vendor]novatek [IC]nt36523w");
			} else {
