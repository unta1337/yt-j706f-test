...
#define PENEL_DIRECTION "nvt_panel_direction"
...
#define READ_LEFT_UP   2
#define READ_RIGHT_UP  3
//write
#define WRITE_VERTICAL   1
#define WRITE_LEFT_UP   2
#define WRITE_RIGHT_UP  3
...
static ssize_t nvt_gesture_store(struct file *file, const char *buffer, size_t count, loff_t *pos) {
	...
	p530_gesture_flag = ts->gesture_enabled;
...
int32_t nvt_edge_reject_set(int32_t status) {
	...
	if(status == WRITE_VERTICAL)//rotate 0/180 degree
		ret = nvt_cmd_store(EDGE_REJECT_VERTICLE_CMD);
	else if(status == WRITE_LEFT_UP) //rotate 90 degree
		ret = nvt_cmd_store(EDGE_REJECT_LEFT_UP);
	else if(status == WRITE_RIGHT_UP) //rotate 270 degree
...
static ssize_t nvt_edge_reject_store(struct file *file, const char *buffer, size_t count, loff_t *pos) {
	...
	if(nvt_cmd_show(EDGE_REJECT) != state) {
...
static int nvt_edge_reject_show(struct seq_file *sfile, void *v) {
	...
	ret = nvt_cmd_show(EDGE_REJECT);

	NVT_LOG("nvt_edge_reject_show %d!\n", ret);

	if(ret == READ_VERTICAL)
		seq_printf(sfile, "Vertical Direction!\n");
	else if(ret == READ_LEFT_UP)
		seq_printf(sfile, "LEFT_UP\n");
	else if(ret == READ_RIGHT_UP)
		seq_printf(sfile, "RIGHT_UP\n");
	else
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_gesture_entry = proc_create(GESTURE_CONTROL, 0666, NULL, &nvt_gesture_fops);
	...
	if (NVT_proc_edge_reject_entry == NULL) {
		NVT_ERR("create proc/panel_direction Failed!\n");
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/panel_direction Succeeded!\n");
	}
