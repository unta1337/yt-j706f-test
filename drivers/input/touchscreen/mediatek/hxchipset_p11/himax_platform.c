...
const struct mtk_chip_config spi_hxtp_ctrl = {
    .rx_mlsb = 1,
    .tx_mlsb = 1,
    .cs_setuptime = 36,
};
...
*/
	...
	/* for p522 spi signal */
	hx_spi->controller_data = (void *)&spi_hxtp_ctrl;
...
static void himax_common_suspend(struct device *dev)
{
	...
	I("%s: enter\n", __func__);
	printk("csy--hxtp--irq_wake_enable");
	irq_wake_enabled = 1;
...
static void himax_common_resume(struct device *dev)
{
	...
	I("%s: enter\n", __func__);
	printk("csy--hxtp--irq_wake_unable");
	irq_wake_enabled = 0;
...
static int __init himax_common_init(void)
{
	...
	I("%s:%s\n", __func__, mtkfb_lcm_name);
		if (IS_ERR_OR_NULL(mtkfb_lcm_name)) {
			...
			if (strcmp(mtkfb_lcm_name,"hx83102p_fhdp_dsi_vdo_boe_p522_drv") == 0) {
				I("TP info: [Vendor]himax [IC]himax83102p\n");
			} else {
				I("%s:Unknown touch",__func__);
				return -ENODEV;
			}
		}
	#endif
