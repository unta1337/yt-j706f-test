		...
		size_t len, loff_t *pos)
{
	...
	p522_gesture_flag = ts->SMWP_enable;
...
void himax_cable_detect_func(bool force_renew)
{
	...
	connect_status = upmu_is_chr_det();/*USB_detect_flag; upmu_is_chr_det(); */
	ts = private_ts;
...
void himax_esd_recovery(int flag)
{
	if (flag)
		himax_chip_common_resume(private_ts);
	else {
