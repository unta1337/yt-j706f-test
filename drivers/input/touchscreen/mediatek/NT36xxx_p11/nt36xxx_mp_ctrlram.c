...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	org_fs = get_fs();
	set_fs(KERNEL_DS);
	fp = filp_open(file_path, O_RDWR | O_CREAT, 0660);
...
int lct_nvt_tp_selftest_callback(unsigned char cmd)
{
	...
	if (of_property_read_bool(np, "novatek,mp-support-dt")) {
		 ...
		 */
		snprintf(mpcriteria, sizeof(mpcriteria), "novatek-mp-criteria-%04X",ts->nvt_pid);//snprintf scnprintf
	...
	if (TestResult_FW_Rawdata != 0) {
		NVT_LOG("FW Rawdata Test FAIL!");
	}
	if(TestResult_Pen_FW_Raw != 0) {
		NVT_LOG("Pen FW Rawdata Test FAIL!");
	}

	//Noise Test Result
	if (TestResult_Noise != 0) {// TestResult_Noise is -1
		NVT_LOG("Noise Test FAIL!");
	}
	if(TestResult_Pen_Noise != 0) {
		NVT_LOG("Pen Noise Test FAIL!");
	}
