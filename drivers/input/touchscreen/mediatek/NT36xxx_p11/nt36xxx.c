...
info_retry:
	...
	case TP_VENDOR_BOE:
		...
		sprintf(fw_version,"[Vendor]boe,[FW]0x%02x,[IC]nt36523w\n",ts->fw_ver);
		update_lct_tp_info(fw_version,NULL);
		break;
	case TP_VENDOR_INX:
		memset(fw_version,0,sizeof(fw_version));
		sprintf(fw_version,"[Vendor]inx,[FW]0x%02x,[IC]nt36523w\n",ts->fw_ver);
		update_lct_tp_info(fw_version,NULL);
		break;
	}
#else
	snprintf(tp_info_buf, PAGE_SIZE, "[Vendor]unknown,[FW]0x%02x,[IC]nt36523w\n", ts->fw_ver);
...
int32_t nvt_set_charger(uint8_t charger_on_off)
{
	...
	ts->charger_flag = charger_on_off;
...
//						pen_tilt_x, pen_tilt_y, pen_distance, pen_btn1, pen_btn2, pen_battery);
				...
				input_report_key(ts->pen_input_dev, GTP_PEN_BUTTON1, pen_btn1);
				input_report_key(ts->pen_input_dev, GTP_PEN_BUTTON2, pen_btn2);
			...
			input_report_key(ts->pen_input_dev, GTP_PEN_BUTTON1, 0);
			input_report_key(ts->pen_input_dev, GTP_PEN_BUTTON2, 0);
		}
...
static void nvt_restore_cmd_func(struct work_struct *work){
	NVT_LOG("%s++\n", __func__);
#if NVT_USB_DETECT
	nvt_set_charger(ts->charger_flag);
#endif
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if(ts->xbuf == NULL) {
		...
		goto err_malloc_xbuf;
  		...
  		switch (ts->touch_vendor_id) {
  		case TP_VENDOR_BOE:
  				memcpy(ts->boot_update_firmware_name, BOOT_UPDATE_BOE_FIRMWARE_NAME,
  							sizeof(BOOT_UPDATE_BOE_FIRMWARE_NAME));
  				memcpy(ts->mp_update_firmware_name, MP_UPDATE_BOE_FIRMWARE_NAME, sizeof(MP_UPDATE_BOE_FIRMWARE_NAME));
  				break;
  		case TP_VENDOR_INX:
  				memcpy(ts->boot_update_firmware_name, BOOT_UPDATE_INX_FIRMWARE_NAME,
  							sizeof(BOOT_UPDATE_INX_FIRMWARE_NAME));
  				memcpy(ts->mp_update_firmware_name, MP_UPDATE_INX_FIRMWARE_NAME, sizeof(MP_UPDATE_INX_FIRMWARE_NAME));
  				break;
  		default:
	...
	if (ts->pen_support) {
		...
		ts->pen_input_dev->keybit[BIT_WORD(GTP_PEN_BUTTON1)] |= BIT_MASK(GTP_PEN_BUTTON1);
		ts->pen_input_dev->keybit[BIT_WORD(GTP_PEN_BUTTON2)] |= BIT_MASK(GTP_PEN_BUTTON2);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	// Remove if condition in case someone using proc node to enable gesture after suspend
	//if(!ts->gesture_enabled) {
		nvt_irq_enable(true);
	//}
...
void nvt_esd_recovery(int flag)
{
	if (flag)
		nvt_ts_resume(&ts->client->dev);
	else
...
static int32_t __init nvt_driver_init(void)
{
	...
	//Check TP vendor
		...
		if (IS_ERR_OR_NULL(mtkfb_lcm_name)) {
			NVT_LOG("mtkfb_lcm_name ERROR!");
			return -ENOMEM;
		} else {
			if (strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_boe_p522_drv") == 0) {
				touch_vendor_id = TP_VENDOR_BOE;
				NVT_LOG("TP info: [Vendor]boe [IC]nt36523w\n");
			} else if (strcmp(mtkfb_lcm_name,"nt36523w_fhdp_dsi_vdo_inx_p522_drv") == 0) {
  				touch_vendor_id = TP_VENDOR_INX;
  				NVT_LOG("TP info: [Vendor]inx [IC]nt36523w\n");
			} else {
				NVT_LOG("%s:Unknown touch\n",__func__);
				return -ENODEV;
			}
		}
	#endif
