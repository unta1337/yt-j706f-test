...
uint32_t dbg_reg_ary[5] = {fw_addr_fw_dbg_msg_addr, fw_addr_fw_dbg_msg2_addr,
...
static void himax_mcu_set_panel_direction(uint8_t direction)
{
       ...
       if (direction == g_core_fp.fp_read_panel_direction()){
               return;
       }
...
static void himax_mcu_read_FW_ver(void)
{
	...
	sprintf(fw_version,"[Vendor]Himax,[FW]0x%02x,[IC]hx83102\n",ic_data->vendor_touch_cfg_ver);
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	err = g_core_fp.fp_firmware_update_0f(fw_entry);
	if (ret >= 0)
