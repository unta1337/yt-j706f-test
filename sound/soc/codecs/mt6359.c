...
/* add by flm for mic bias switch start [4]*/
#if defined(CONFIG_TARGET_PROJECT_P522)
enum {
	DISABLE_MIC_BIAS_SWITCH,
	ENABLE_MIC_BIAS_SWITCH
};
#endif
/* add by flm for mic bias switch end [4]*/
...
/* add by flm for mic bias switch start [1]*/
#if defined(CONFIG_TARGET_PROJECT_P522)
static const char *const mic_bias_switch_value[] = {
	"Off", "On"
};

static const struct soc_enum mic_bias_switch_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mic_bias_switch_value), mic_bias_switch_value),
};
#endif
/* add by flm for mic bias switch end [1]*/
...
/* add by flm for mic bias switch start [3]*/
#if defined(CONFIG_TARGET_PROJECT_P522)
static int mt6785_mic_bias_switch_get(struct snd_kcontrol *kcontrol,
		      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mt6359_priv *priv = snd_soc_component_get_drvdata(cmpnt);
	...
	regmap_read(priv->regmap, MT6359_AUDENC_ANA_CON15, &value[0]);
	regmap_read(priv->regmap, MT6359_AUDDEC_ANA_CON13, &value[1]);
	if ((value[0] & 0x21) == 0x21 && (value[1] & 0x10) == 0x0)
		ucontrol->value.integer.value[0] = 1;
	else
		ucontrol->value.integer.value[0] = 0;

	dev_info(priv->dev, "%s() MT6359_AUDENC_ANA_CON15 = 0x%x, MT6359_AUDDEC_ANA_CON13 = 0x%x", __func__, value[0], value[1]);
...
static int mt6785_mic_bias_switch_set(struct snd_kcontrol *kcontrol,
		      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mt6359_priv *priv = snd_soc_component_get_drvdata(cmpnt);
	...
	switch (index) {
		case ENABLE_MIC_BIAS_SWITCH:
				dev_info(priv->dev, "%s(), index %d\n", __func__, index);
				regmap_update_bits(priv->regmap, MT6359_AUDDEC_ANA_CON13,
						   RG_AUDGLB_PWRDN_VA32_MASK_SFT,
						   0 << RG_AUDGLB_PWRDN_VA32_SFT);
				regmap_write(priv->regmap, MT6359_AUDENC_ANA_CON15, 0x0021);
				break;
		case DISABLE_MIC_BIAS_SWITCH:
				dev_info(priv->dev, "%s(), index %d\n", __func__, index);
				regmap_update_bits(priv->regmap, MT6359_AUDDEC_ANA_CON13,
						   RG_AUDGLB_PWRDN_VA32_MASK_SFT,
						   1 << RG_AUDGLB_PWRDN_VA32_SFT);
...
#endif
/* add by flm for mic bias switch end [3]*/
...
static const struct snd_kcontrol_new mt6359_snd_ul_controls[] = {
	...
	MT_SOC_ENUM_EXT_ID("Mic_Type_Mux_2", mic_type_mux_enum[0],
			   mic_type_get, mic_type_set,
			   MUX_MIC_TYPE_2),
	/* add by flm for mic bias switch start [2] */
#if defined(CONFIG_TARGET_PROJECT_P522)
	SOC_ENUM_EXT("Mic_Bias_Switch", mic_bias_switch_enum[0],
			   mt6785_mic_bias_switch_get, mt6785_mic_bias_switch_set),
#endif
	/* add by flm for mic bias switch end [2]*/
};
