...
static uint16_t aw881xx_chipid = 0;
...
int aw881xx_sysst_check(struct aw881xx *aw881xx)
{
	int ret = -1;
	unsigned char i;
	uint16_t reg_val = 0;
...
static int aw881xx_syspll_check(struct aw881xx *aw881xx)
{
        int ret = -1;
        aw_dev_dbg(aw881xx->dev, "%s: enter\n", __func__);
        aw_dev_info(aw881xx->dev, "%s: 0709 enter\n", __func__);
				...
				const struct i2c_device_id *id)
{
	...
	dapm_widgets = devm_kzalloc(&i2c->dev, sizeof(struct snd_soc_dapm_widget) * ARRAY_SIZE(aw881xx_dapm_widgets), GFP_KERNEL);
	if (!dapm_widgets)
		return -ENOMEM;

	memcpy(dapm_widgets, aw881xx_dapm_widgets, sizeof(struct snd_soc_dapm_widget) * ARRAY_SIZE(aw881xx_dapm_widgets));
	/* change dapm */
	ret = aw881xx_append_suffix("%s_%x_%x",
		&dapm_widgets[0].sname, aw881xx);
	if (ret < 0)
		return ret;

	ret = aw881xx_append_suffix("%s_%x_%x",
		&dapm_widgets[2].sname, aw881xx);
	if (ret < 0)
		return ret;

	soc_codec_dev_aw881xx.component_driver.dapm_widgets = dapm_widgets;
	...
	aw_dev_info(&i2c->dev, "%s : rx dapm :%s\n",
				__func__, soc_codec_dev_aw881xx.component_driver.dapm_widgets[0].sname);
				
	aw_dev_info(&i2c->dev, "%s : tx dapm :%s\n",
				__func__, soc_codec_dev_aw881xx.component_driver.dapm_widgets[2].sname);
...
uint16_t aw881xx_get_chipid(void) {
	return aw881xx_chipid;
}
EXPORT_SYMBOL_GPL(aw881xx_get_chipid);
...
static int __init aw881xx_i2c_init(void)
{
	...
	else if(aw881xx_chipid != AW881XX_CHIPID) {
		i2c_del_driver(&aw881xx_i2c_driver);
		pr_err("%s: aw881xx chipid(0x%x) fail del i2c init\n", __func__, aw881xx_chipid);
	}
