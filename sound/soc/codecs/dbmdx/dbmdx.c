	...
	if (p->pdata->feature_va_ve) {
		...
		if (p->va_chip_enabled) {
			...
			if (p->active_interface_type_va != DBMDX_CMD_INTERFACE) {
				return snprintf(buf, PAGE_SIZE, "VA chip is not in a cmd mode\n");
			}
				ret = dbmdx_switch_to_va_chip_interface(p,
					DBMDX_CMD_INTERFACE);
				if (ret) {
		...
		if (p->va_ve_chip_enabled) {
			...
			if (p->active_interface_type_va_ve != DBMDX_CMD_INTERFACE) {
				return snprintf(buf, PAGE_SIZE, "VA_VE chip is not in a cmd mode\n");
			}
				ret = dbmdx_switch_to_va_ve_chip_interface(p,
					DBMDX_CMD_INTERFACE);
				if (ret) {
...
out:
...
#else
	int ret;
	dev_info(p->dev, "%s: ken add patch to Reboot Start\n", __func__);
	...
	if (ret != 0) {
	    dev_info(p->dev, "%s: reboot fail\n", __func__);
	    return 1;
	} else {
	    dev_info(p->dev, "%s: ken add patch to Reboot done\n", __func__);
	    return 0;
	}
#endif
}
...
static int input_source_get(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	ucontrol->value.integer.value[0] = p->input_source;
...
static int input_source_set(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	p->input_source = ucontrol->value.integer.value[0];
...
static const DECLARE_TLV_DB_SCALE(VOW_input_source, 0, 1, 0);
...
static const struct snd_kcontrol_new dbmdx_va_ve_snd_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("INPUT_SOURCE", SND_SOC_NOPM,
		0, 2000, 0, input_source_get, input_source_set, VOW_input_source),
};
