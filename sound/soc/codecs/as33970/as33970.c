...
extern int mtk_i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num,
					unsigned int ext_flag, unsigned int timing);

static int as33970_set_i2c_master_speed(struct snd_soc_codec *codec,
			     unsigned int speed_hz)
{
	struct as33970_priv *as33970 = snd_soc_codec_get_drvdata(codec);
	struct i2c_client *i2c = to_i2c_client(as33970->dev);
	struct i2c_msg msg[1];
	unsigned char data[6] = {0, 4, 0, 0, 0, 0};
	int ret = 0;

    msg[0].addr = AS33970_I2C_ADDRESS;
    msg[0].flags = 0; //7 bits
    msg[0].len = 6;
    msg[0].buf = (unsigned char *)data;
	dev_info(as33970->dev, "%s set i2s speed %dhz", __func__, speed_hz);
	ret = mtk_i2c_transfer(i2c->adapter, msg, 1, 0, speed_hz);
	if (ret < 0) {
		as33970->is_i2c_speed_set = 0;
		dev_err(as33970->dev, "Failed to set i2c master clock, error no %d\n", ret);
	} else {
		as33970->is_i2c_speed_set = 1;
	}
										...
										unsigned int trigger_model)
{
	struct as33970_cmd cmd;
	int ret = 0;
	dev_info(as33970->dev, "as33970_set_always_listening is_always_listening %d\n", is_always_listening);
	if (is_always_listening) {
		...
		dev_info(as33970->dev, "as33970_set_always_listening fw_is_always_listening %d\n", as33970->fw_is_always_listening);

		if (!as33970->fw_is_always_listening || (as33970->fw_trigger_model != trigger_model) ) {
			if(trigger_model == GOOGLE_SOURCE) {
				dev_info(as33970->dev, "as33970_set_always_listening  0\n");
				ret = as33970_sendcmd(as33970, &cmd,
				...
				if (ret < 0)
					return ret;
				msleep(20);
				dev_info(as33970->dev, "as33970_set_always_listening GOOGLE_SOURCE  1\n");
				ret = as33970_sendcmd(as33970, &cmd,
								AS33970_CMD_SET(SYS_CMD_EVENT_PARAM),
								AS33970_ID_ARM, 7,
								EVENT_LP_TRIG_CONTROL, EVENT_PAR_LP_TRIG_STATE, 1,EVENT_PAR_LP_TRIG_GOOG_EN,1,EVENT_PAR_LP_TRIG_SENS_EN, 0);
			} else if(trigger_model == CLOVA_SOURCE) {
				dev_info(as33970->dev, "as33970_set_always_listening  0\n");
				ret = as33970_sendcmd(as33970, &cmd,
								AS33970_CMD_SET(SYS_CMD_EVENT_PARAM),
								AS33970_ID_ARM, 3,
				...
				if (ret < 0)
					return ret;
				msleep(20);
				dev_info(as33970->dev, "as33970_set_always_listening CLOVA_SOURCE  1\n");
				ret = as33970_sendcmd(as33970, &cmd,
								AS33970_CMD_SET(SYS_CMD_EVENT_PARAM),
								AS33970_ID_ARM, 7,
								EVENT_LP_TRIG_CONTROL, EVENT_PAR_LP_TRIG_STATE, 1,EVENT_PAR_LP_TRIG_GOOG_EN,0,EVENT_PAR_LP_TRIG_SENS_EN, 1);
			} else {
				dev_info(as33970->dev, "Can't find trigger_model %d \n",trigger_model);
			}

			if (ret < 0)
				return ret;
			as33970->fw_trigger_model = trigger_model;
...
static ssize_t store_reset_dsp(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	struct as33970_priv *as33970 = dev_get_drvdata(dev);
...
static ssize_t store_fw_status(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	struct as33970_priv *as33970 = dev_get_drvdata(dev);
	struct as33970_cmd cmd;
	int ret = 0;
								...
								struct snd_ctl_elem_value *ucontrol)
{
	...
	if (as33970->is_record_enabled && AUDIO_HOT_WORD != as33970->output_signal) {
		...
		dev_dbg(as33970->dev, "Recording in none trigger mode, record the trigger status\n");
		mutex_unlock(&as33970->cmd_mutex);
		return ret;
	}
										...
										struct snd_ctl_elem_value *ucontrol)
{
	...
	case AUDIO_RECOGNITION:
		...
		if (as33970->trigger_model != CLOVA_SOURCE) {
			ret = as33970_sendcmd(as33970, &cmd,
							AS33970_CMD_SET(SYS_CMD_EVENT_PARAM),
							AS33970_ID_ARM, 3,
							EVENT_LP_TRIG_CONTROL, EVENT_PAR_LP_TRIG_STATE, 0);
...
static int fw_status_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);
	ucontrol->value.integer.value[0] = as33970->fw_status;
	return 0;
}

static int fw_status_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);
	as33970->fw_status = (unsigned int)ucontrol->value.integer.value[0];
	if (1 == as33970->fw_status) {
		as33970->mode = 0;
		as33970->is_record_enabled = 0;
		if (as33970->is_always_listening) {
			as33970->fw_is_always_listening = 0;
			as33970->fw_is_high_performance_trigger = 0;
			as33970_set_always_listening(as33970, 
				as33970->is_always_listening,
				as33970->is_high_performance_trigger,
				as33970->trigger_model);
			as33970->output_signal = AUDIO_HOT_WORD;
		} else {
			as33970_set_mode(as33970, AS33970_MODE_NORMAL);
			as33970_set_record_effect(as33970, as33970->is_aec_enabled, as33970->is_nr_enabled);
			as33970->output_signal = AUDIO_COMMUNICATION;
		}
	}
	dev_info(as33970->dev, "fw_status_set %d", as33970->fw_status);
	return 0;
}


static int usb_status_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);
	ucontrol->value.integer.value[0] = as33970->usb_status;
	return 0;
}

static int usb_status_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);
	as33970->usb_status = (unsigned int)ucontrol->value.integer.value[0];
	return 0;
}

extern unsigned int get_i2c_speed(int id);
extern void set_i2c_speed(int id, unsigned int speed_hz);
extern void dump_i2c_status(int id);
static unsigned int as33970_i2c_speed = 400 * 1000;//hz
#define AS33970_I2C_ID (9)
static int i2c_speed_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	unsigned int i2c_speed = 0;

	dump_i2c_status(AS33970_I2C_ID);
	i2c_speed = get_i2c_speed(AS33970_I2C_ID);
	ucontrol->value.integer.value[0] = i2c_speed;
	printk("as33970 i2c-9 speed %dhz(%dhz)", i2c_speed, as33970_i2c_speed);
	return 0;
}

static int i2c_speed_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	as33970_i2c_speed = (unsigned int)ucontrol->value.integer.value[0];

	set_i2c_speed(AS33970_I2C_ID, as33970_i2c_speed);

	return 0;
}

static const DECLARE_TLV_DB_SCALE(i2c_speed_hz, 100 * 1000, 100 * 1000, 0);

static int music_volume_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);

	ucontrol->value.integer.value[0] = as33970->music_volume;
	return 0;
}

static int music_volume_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct as33970_priv *as33970 = snd_soc_component_get_drvdata(component);

	as33970->music_volume = (unsigned int)ucontrol->value.integer.value[0];

	return 0;
}

static const DECLARE_TLV_DB_SCALE(music_volume_index, 0, 1, 0);
 ...
 static void as33970_work_queue(struct work_struct *work)
{
	...
	mutex_lock(&as33970->cmd_mutex);
	if (as33970->is_record_enabled) {
		as33970_enable_record(as33970, as33970->is_record_enabled);
	} else {
...
static void as33970_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	dev_info(as33970->dev, "%s exit, playback = %d, record = %d\n", __func__, as33970->is_playback_enabled, as33970->is_record_enabled);
	mutex_unlock(&as33970->cmd_mutex);
}
...
static const char * const as33970_fac_test[] = {
		"Off", "MIC1", "MIC2", "Enable_Echo REF", "Trigger"};
static const struct soc_enum fac_test_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(as33970_fac_test),
				as33970_fac_test),
};

static const char * const fw_status_value[] = {"FAIL", "PASS", "ONGOING"};
static const struct soc_enum fw_status_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(fw_status_value), fw_status_value),
};

static const char * const usb_status_value[] = {"Off", "On"};
static const struct soc_enum usb_status_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(usb_status_value), usb_status_value),
};
...
static const struct snd_kcontrol_new as33970_snd_controls[] = {
	...
	SOC_ENUM_EXT("AS33970 FW", fw_status_enum[0], fw_status_get, fw_status_set),
	SOC_SINGLE_EXT_TLV("I2c_Speed", SND_SOC_NOPM, 0, 1000 * 1000, 0, i2c_speed_get, i2c_speed_set, i2c_speed_hz),
	SOC_ENUM_EXT("USB Status", usb_status_enum[0], usb_status_get, usb_status_set),
	SOC_SINGLE_EXT_TLV("AS33970 Music Volume", SND_SOC_NOPM, 0, 15, 0, music_volume_get, music_volume_set, music_volume_index),
};
...
static irqreturn_t irq_handler(int irq, void *data){
	...
	if (!as33970->is_record_enabled) {
		as33970->trigger_source = as33970->fw_trigger_model;
		dev_info(as33970->dev, "Trigger %d detected\n", as33970->trigger_source);
	} else {
			     ...
			     const struct i2c_device_id *id)
{
...
#ifdef AS33970_ENABLE_PIN
	as33970->enable_gpio = of_get_named_gpio(np, "enable-gpio", 0);
	if (as33970->enable_gpio < 0)
		dev_err(dev, "as33970 get enable-gpio failed\n");
	
	if (gpio_is_valid(as33970->enable_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, as33970->enable_gpio,
			GPIOF_OUT_INIT_HIGH, "AS33970_ENABLE_GPIO");
		if (ret)
			return ret;
	}
	dev_info(dev, "%s: enable_gpio = %d.\n", __func__, as33970->enable_gpio);
	gpio_set_value_cansleep(as33970->enable_gpio, 1);
#endif
	...
	if (gpio_is_valid(as33970->rst_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, as33970->rst_gpio,
			GPIOF_OUT_INIT_HIGH, "AS33970_RST_GPIO");
		if (ret)
			return ret;
	}
	...
	as33970->irq_gpio = of_get_named_gpio(np, "irq-gpios", 0);
	if (gpio_is_valid(as33970->irq_gpio)) {
		...
		ret = request_irq(gpio_to_irq(as33970->irq_gpio), irq_handler, IRQF_TRIGGER_RISING, "soundtrigger_irq", as33970);
		if (!ret) {
	...
	ret = devm_snd_soc_register_component(as33970->dev, 
	...
	if (ret < 0)
		...
		dev_dbg(dev, "%s: Register codec 20211018.\n", __func__);
